import React,{Component} from 'react';
import {Button,Icon,Checkbox,InputNumber } from 'antd';
import {Redirect} from 'react-router-dom';
import './index.scss';
import axios from 'axios';


class Cart extends Component{
	constructor(){
		super();
		this.state={
			shopType:false,//购物车为空
			shopArr:[],
			play:false,
			tokenType:1
			
		}
	}
	
	//对于是否已经登录的判断我们放在componentWillMount
	
	
	componentDidMount(){
		//token的验证
		axios({
			method:'get',
			url:'http://192.168.2.251:7001/center',
			headers:{
				'Authorization':localStorage['token']
			}
		}).then((res)=>{
			console.log(res);
			if(res.data.code===0){
				//登录状态获取成功
				this.setState({
					tokenType:true
				})
			}else{
				//this.props.history.push('/login')
				this.setState({
					tokenType:false
				})
			}
			console.log(this.state.tokenType);
		})
		
		//判断购物车是否为空
		if(localStorage.getItem('cart')){
			let arr=JSON.parse(localStorage.getItem('cart'));
			if(arr!=null&&arr.length){
				this.setState({
					shopType:true,
					shopArr:arr
				})
			}
		}
	}
	
	goHome=()=>{
		this.props.history.push('/');
		
	}
	gogoing=()=>{
		this.props.history.go(-1);
	}
	
	dele=(e)=>{
		var index=e.target.getAttribute("data-index");
		var myshopArr=this.state.shopArr;
		myshopArr.splice(index,1);
		this.setState({shopArr:myshopArr});
		console.log(myshopArr)
		localStorage.setItem('cart',myshopArr)
		
	}
	
	render(){
			function onChange(value) {
  				console.log('changed', value);
 
			}
			
			
			if(this.state.tokenType){
				return(
			<div className='cart'>
				<div className='cart_title'>
					<Icon onClick={this.gogoing} type="left" />
					<span>
						购物车
					</span>
					<span >
						编辑
					</span>
				
				</div>
				<ul className='zhengpin'>
					<li>
						<img src='https://image.watsons.com.cn//upload/2e8ebc1f.png' alt=''/>
						正品保证
					</li>
					<li>
						<img src='https://image.watsons.com.cn//upload/278ce554.png' alt=''/>
						屈臣氏品牌
					</li>
					<li>
						<img src='https://image.watsons.com.cn//upload/2e8ebc1f.png' alt=''/>
						7天退货
					</li>
				</ul>
				<div className='cart_txt'>
					尊敬的屈臣氏用户：我们对《隐私政策》进行了更新。请仔细阅读《隐私政策》并确定了解我们对您个人信息的处理规则。
					阅读过程中，如您有任何疑问，可联系我们的客服咨询，如您不同意协议中的任何条款，您应立即停止继续使用屈臣氏提供的服务。【点击查阅】
				</div>
				
				
				
				<div className='cart_content'>
					{
						this.state.shopType?<ul>
						{
							this.state.shopArr.map((item,index)=><li key={index}>
								
								  <Checkbox ></Checkbox>
								
								<img src={item.img_src} alt={item.name}/>
								<div className='xiangqin'>
									<div>
										<div>
											{item.name}
											<div className='del'>
												<div onClick={this.dele}>  
													<Icon type="rest" />删除
												</div>
												
													<InputNumber style={{'color':'red','fontSize':'16px'}} size="large" min={1} max={10} defaultValue={1} onChange={onChange} />
												
											</div>	
											
										</div>
										
										
									</div>
									<div>
										<span>￥{item.app_price/100}</span>
										<span>￥{item.market_price/100}</span>
										<span>x{item.num}</span>
									</div>
								</div>
								
							</li>)
						}
						</ul>:<div className='cart-tishi'>
							<div>
								<img src="data:image/png;base64," alt=''/>
							
							</div>
							<div>剁剁剁,装满购物车</div>
							<div>
								<Button type='primary' className='cart_btn' onClick={this.goHome}>去首页逛逛</Button>
							</div>
						</div>	
					}
				</div>
			</div>
		)
			}else{
				return <Redirect to='/login'/>
			}
			
			
		
	}
	
}

export default Cart;